...
;(function($) {
	jQuery.fn.fancyNews = function(arg) {
		...
		var $elem, newsContainers, $newsList, $nav, previewSize, slideAmount, slideInt, contents = [], topIndex = 0, listItemHeight = false;

			function _init(elem) {

				$elem = $(elem).addClass('fn-container').css({color: options.textColor});
				...
				.append('<div class="fn-full-post" style="background:'+options.backgroundColor+'"><h2 style="color: '+options.primaryColor+';"></h2><div></div></div>');

				$newsList = $elem.children('.fn-news-list');

				//save preview height
				previewSize = options.vertical ? $elem.width() / options.previewsPerPage : $elem.height() / options.previewsPerPage;
				listItemHeight = options.vertical ? $elem.height() : previewSize;
				previewSize -= options.offset;
				slideAmount = previewSize + options.offset;

				//load rss feed if URL is passed
				if(options.feed){
					if(!options.rssApiKey || options.rssApiKey == '') {
						alert('Please enter an API key to load the RSS feed. You can get an API key here: https://rss2json.com');
					}
					else {
						_loadRssFeed();
					}
			...
			function _loadFromDom(){
				...
				for(var i=0; i < newsContainers.length; ++i){
					...
					_appendNewsItem(
						$(newsContainers[i]).children('span,div').attr('title').length > 0 ? $(newsContainers[i]).find('span,div').attr('title') : '',
						newsContainers[i].title,
						$(newsContainers[i]).children('span,div').text(),
						options.useLinks ? $(newsContainers[i]).data('link') : $(newsContainers[i]).children('span,div').html(),
			...
			function _loadRssFeed() {
				...
				$.ajax({
			        ...
			        dataType: 'json',
			        data: {
			            rss_url: options.feed,
			            api_key: options.rssApiKey,
			            count: options.numOfEntries
			        }
				}).done(function (response) {
				    if(response.status != 'ok'){
						$elem.append('<p class="fn-error"><strong>Error!</strong> The URL of the RSS feed is not correct or could not be loaded.</p>');
					}
					else {
						response.items.forEach(function(item) {
							_appendNewsItem(
								item.title,
								String(item.pubDate).substr(0,16),
								item.content.replace(/(<([^>]+)>)/ig,""),
								options.useLinks ? item.link : item.content,
								item.thumbnail
							);
						});
			...
			function _appendNewsItem(title, date, excerpt, content, image) {
				...
				var dateHtml = date != undefined && date != '' ? '<h4>'+date+'</h4>' : '',
					imageHtml = image != undefined && image != '' ? '<img src="'+image+'" style="border-color: '+options.primaryColor+';" />' : '';
				...
				var $lastItem = $newsList.append('<li class="fn-preview fn-clearfix" style="background:'+options.backgroundColor+';" data-index="'+$newsList.children('li').size()+'">'+imageHtml+'<div><h3 style="color: '+options.primaryColor+';">'+title+'</h3>'+dateHtml+'<p></p></div></li>').children('li:last');

				var excerptWords = excerpt.split(" "),
					$wrapper = $lastItem.children('div'),
					$excerpt = $wrapper.children('p');
				...
				//append words depending on height
				for(var i=0; i < excerptWords.length; ++i) {
					var wrapperSize = options.vertical ? ($wrapper.get(0).offsetHeight + $wrapper.position().top) : $wrapper.get(0).offsetHeight;
					if(wrapperSize < listItemHeight-10) {
						$excerpt.append(excerptWords[i] + ' ');
					}
					else {
						break;
					}
				...
				//remove last words and add custom string to the end
			//	var currentString = $excerpt.text();
			//	for(var j=0; j < 4; ++j) {
			//		var lastIndex = currentString.lastIndexOf(' ');s
			//		currentString = currentString.substring(0, lastIndex);
			//	}
			//	$excerpt.text(currentString + ' ' + options.excerptEnd);

				//list item handlers
				$lastItem.delay(100+($newsList.children('li').size() * 100)).animate({opacity: 1}, 500);
			...
			function _listCreated(){
				...
				$elem.after('<div class="fn-navigation fn-clearfix" style="color: '+options.primaryColor+'; background:'+options.backgroundColor+'"><span class="fn-post-count">1 - '+options.previewsPerPage+'</span><span class="fn-close-button fa fa-times"></span></div>');

				$nav = $elem.next('.fn-navigation').width(options.width);
				...
				if(options.vertical) {
					$nav.addClass('fn-vertical').width(options.width);
				}

				if(options.arrows) {
					var arrowHtml = options.vertical ? '<span class="fn-next-button fa fa-arrow-right"></span><span class="fn-previous-button fa fa-arrow-left"></span>' : '<span class="fn-previous-button fa fa-arrow-up"></span><span class="fn-next-button fa fa-arrow-down"></span>';
				...
				.delegate('li', 'click', function(){
					...
					if(options.useLinks) {
						...
						.children('h2').text($this.children('div').children('h3').text())
						.parent().children('div').html(content).fadeOut(0);

						var titleHeight = $fullPostElem.children('h2').height() + parseInt($fullPostElem.children('h2').css('marginTop')) + parseInt($fullPostElem.children('h2').css('marginBottom'));
	...
	$.fn.fancyNews.defaults = {
		...
		backgroundColor: '#ECF0F1', //the background color
		backgroundOverColor: '#F3F8F9', //the background color when you move your mouse over a preview
		...
		excerptEnd : '[....]', //V2.0.2 - the string that should be put at the end of the excerpt
		textColor: '#333333', //V2.0.2 - hex color for the text
		primaryColor: '#7F8C8D', //V2.0.2 - primary color
		center: false, //V2.0.2 - center slider
		rssApiKey: '' //V2.0.4 - get an API key to load rss feeds https://rss2json.com
	};
})(jQuery);
